...
load_data <- function(file, strand, name) #function to load data from input files into a data frame
{
  ...
  if(ncol(input_data) == 5) {                                                       #test if there are 5 columns in the input file (no sd)
    coverage <- data.frame(input_data [,4], input_data[,5], name, strand_value, 0, row.names = NULL)  #sd is set at 0
    colnames(coverage) <- c('position', 'data', 'name', 'strand', 'sd')
  } else {
    coverage <- data.frame(input_data [,4], input_data[,5], name, strand_value, as.numeric(input_data[,6]), row.names = NULL) #sd loaded from file
    colnames(coverage) <- c('position', 'data', 'name', 'strand', 'sd')      
    }
  return(coverage)
}
...
reselect_datasets_tree <- function(data_list_item, tree_list, coverage_plot_globallist, wc_globallist){   #change displayed datasets based on tree selection take a data frame to modify
  ...
  if(length(added_data) > 0){
    #browser()
    for(i in added_data){
      if(!is.na(data_to_load$filename_plus[data_to_load$samplename == i])) coverage_plot_globallist[[data_list_item]]  <- rbind(coverage_plot_globallist[[data_list_item]], load_data(data_to_load$filename_plus[data_to_load$samplename == i], "+", i))
      if(!is.na(data_to_load$filename_minus[data_to_load$samplename == i])) coverage_plot_globallist[[data_list_item]] <- rbind(coverage_plot_globallist[[data_list_item]], load_data(data_to_load$filename_minus[data_to_load$samplename == i], "-", i))
    }
