...
generate_one_plot <- function(plot_number, coverage_plot_globallist, wc_globallist) {
  ...
  }, cacheKeyExpr = {c(position$start,  position$end, input$bin_size, input$zoom, input[[paste0('plot', plot_number, '_scale')]], input[[paste0('plot', plot_number, '_norm')]], outputs$position, outputs$yvalue, input[[paste0('plot', plot_number, '_diff')]], unique(coverage_plot_globallist[[plot_number]]$name) )})
}
