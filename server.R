...
output$users_number = renderUI({
  p(paste0("Current number of active users: ", users$count))
})
    ...
    for(dd in 1:nrow(default_datasets)) {                                 #Go through all rows representing default datasets; use for loop to return dataframe ready for ggplot2
      if(!is.na(default_datasets$filename_plus[dd])){                                                                     #if filename for plus strand exists, load the file
        if(nrow(coverage_plot) == 0) coverage_plot <- load_data(default_datasets$filename_plus[dd], "+", default_datasets$samplename[dd])     #If first dataset just load
          else coverage_plot <- bind_rows(coverage_plot, load_data(default_datasets$filename_plus[dd], "+", default_datasets$samplename[dd]))     #If some data exist, add
      ...
      if(!is.na(default_datasets$filename_minus[dd])){                                                                     #if filename for minus strand exists, load the file
        ...
        else coverage_plot <- bind_rows(coverage_plot, load_data(default_datasets$filename_minus[dd], "-", default_datasets$samplename[dd]))
...
#Scrolling and zooming of the plots
  ...
  observeEvent(input$bin_size, {                        #zoom in or out
    if(input$bin_size > 1000) {
      updateSliderInput(session, "bin_size", value = 1000)
      showModal(modalDialog("Cannot use more than 1000 bins"))
    }
  })
...
lapply(1:number_of_plots, function(i){  
  output[[paste0('tree', i)]] <- jsTreeR::renderJstree({         #load tree created with jsTree with data from infiles.csv
    tree_data <- read.table('infiles.csv', sep=",", header=TRUE, stringsAsFactors = FALSE)
    makelist <- function(df){   
      children <- unique(df[,1])
      a <- lapply(children, function(current_level, df){
        b <- list(text = current_level, state = list(disabled = TRUE, opened = TRUE))
        b$children <- lapply(unique(df[df[,1] == current_level, 2]), function(current, df){
          c <- list(text = current, state = list(disabled = TRUE))
          c$children <- lapply(unique(df[df[,2] == current, 3]), function(current2) {
            if(df[df[,3] == current2, 6] != i) return(list(text = current2))
            if(df[df[,3] == current2, 6] == i) return(list(text = current2, state = list(selected = TRUE)))
          })
          return(c)
        }, df = df)
        return(b)
    ...
    nodes <- makelist(tree_data)
    jstree(nodes, checkboxes = TRUE, theme = "proton", selectLeavesOnly = TRUE, multiple = TRUE, sort = FALSE)
  })  
  })  
...
lapply(1:number_of_plots, function(i){  
  observeEvent(input[[paste0('tree', i, '_selected')]], {            #load the list os selected datasets from the tree created with jsTree
    #browser()
    tmp <- input[[paste0('tree', i, '_selected')]]
    if(length(tmp) > 0){
      if(length(tmp) > 5) showModal(modalDialog("Selecting more than 5 datasets is not recommended"))
      current_selection <- as.data.frame(matrix(unlist(tmp),nrow=length(tmp),byrow=TRUE))
      colnames(current_selection) = "col1"
      current_selection <- drop_na(current_selection)   #remove rows with NAs - contain partial tree info
      dataset_list <- reselect_datasets_tree(i, current_selection$col1, coverage_plot_globallist, wc_globallist)
...
generate_one_plot <- function(plot_number, coverage_plot_globallist, wc_globallist) {
  if(input$bin_size > 1000) updateSliderInput(session, "bin_size", value = 1000)                  #Use cache
  if(input[[paste0('plot', plot_number, '_type')]] == 2) output[[paste0('Plot', plot_number)]] <- renderCachedPlot({   #DNA plot
    req(input$bin_size)
    #browser()
  ...
  }, cacheKeyExpr = {c(plot_number, position$start,  position$end, input$bin_size, input$zoom, input[[paste0('plot', plot_number, '_scale')]], input[[paste0('plot', plot_number, '_norm')]], outputs$position, outputs$yvalue, input[[paste0('plot', plot_number, '_diff')]], unique(coverage_plot_globallist[[plot_number]]$name) )})
}


#Generate navigation plot
output$naviPlot <- renderCachedPlot({             #navigation plot
  plot_navi(position$start, position$end, annotation, operons)
}, cacheKeyExpr = {c(position$start,  position$end)})
