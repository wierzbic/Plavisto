...
shinyUI(fluidPage( 
  title = "Plavisto",
  h2("Plavisto"),
  h4(a("Wierzbicki Lab", href="https://sites.lsa.umich.edu/wierzbicki-lab/"), 'Plastid Genome Visualization Tool'),
      ...
      tabPanel("Browser",
          ...
          fluidRow( 
              column(4,
                  ...
                  numericInput("size", "Shown Region Size", 5000, 100, genome_length, 100),
                  numericInput("bin_size", "Number of bins", 500, 10, 2000, 100),
                  numericInput("zoom", "Zoom", 1, 1, 100, 1)
                  #numericInput("plot_height", "Plot height", 250, 10, 1000, 1)
                  ),
              column(3,
                  actionButton("ScrollL", "Scroll Left"),
                  actionButton("ScrollR", "Scroll Right"),
                  radioButtons(paste0('plot', 1, '_type'), paste0('Select type of plot ', 1), choices = c("Show plot" = 2, "No plot" = 3), inline = 1, selected = 2),
                  lapply(2:number_of_plots, function(i){  
                    radioButtons(paste0('plot', i, '_type'), paste0('Select type of plot ', i), choices = c("Show plot" = 2, "No plot" = 3), inline = 1, selected = 3)
                  })
                  ),
              column(5,
